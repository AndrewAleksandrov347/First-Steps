...
const initialHumanSpawnInterval = 200;
let humanSpawnInterval = initialHumanSpawnInterval;
const minHumanSpawnInterval = 60;
...
function updateGorilla() {
    ...
    if (gorilla.heldHuman) {
      ...
      for (let i = 0; i < humans.length; i++) {
        ...
        if (!human.isGrabbed && !human.isThrown) {
          if (
            human.x < grabZoneX + grabZoneWidth &&
  ...
  if (gorilla.heldHuman) {
    gorilla.heldHuman.x =
      gorilla.x + gorilla.width - gorilla.heldHuman.width / 2;
    gorilla.heldHuman.y =
      gorilla.y + gorilla.height / 2 - gorilla.heldHuman.height / 2;
  }
}
...
function spawnHuman() {
  ...
  if (humanSpawnTimer >= humanSpawnInterval) {
    ...
    const currentSpeed = Math.min(
      humanProto.maxSpeed,
      humanProto.initialSpeed + (score / 120) * humanProto.speedIncrement
    );
...
function updateDifficulty() {
  humanSpawnInterval = Math.max(
    minHumanSpawnInterval,
    initialHumanSpawnInterval - Math.floor(score / 40) * 5
  );
...
function updateHumans() {
  ...
  for (let i = humans.length - 1; i >= 0; i--) {
    ...
    if (human.isThrown) {
      ...
      if (
        human.x > canvas.width + human.width ||
        human.y > canvas.height ||
        human.y + human.height < 0
      ) {
        humans.splice(i, 1);
      }
    } else {
